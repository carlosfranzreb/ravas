...
DIRECTORY = os.path.realpath(
    os.path.join(os.path.dirname(__file__), "..", "..", "..", "..", "rpm", "build")
)
...
def handler_for_dir(directory: str):
    ...
    return type(
        f"HandlerForDirectory<{directory}>",
...
def start_server(
    ...
    with socketserver.TCPServer(("", port), handler_for_dir(directory)) as httpd:
        logger.info(
            "starting to serve at port %s, contents of directory %s", port, directory
        )
        try:
            httpd.serve_forever()
...
if __name__ == "__main__":
    parser = ArgumentParser(
        "Start static file server for React-App (for Avatar Rendering)"
    )
    ...
    try:
        ...
        logger.error(
            "Encountered error when starting server", exc_info=exc, stack_info=True
        )
