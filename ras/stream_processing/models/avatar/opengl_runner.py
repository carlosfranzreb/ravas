...
def init_render_window_and_load_data(
    model_path: str, blendshapes_data_path: str, headless: bool = False
) -> WindowConfig:
    config = init_render_window(headless)
    win: GLTFRenderer = config.wnd
    win.load_model(model_path)
...
def init_render_window(headless: bool, log_level=None):
    ...
    if headless:
        ...
        win_args = (
            "--cursor",
            "True",
        )  # <- set the cursor via args, so that it is not empty
...
def start_renderer(
    ...
    try:
        ...
        logging.getLogger().error(
            "ERROR for open gl render: %s", exc, exc_info=exc, stack_info=True
        )
...
if __name__ == "__main__":
    ...
    parser = ArgumentParser(
        description="run OpenGL based avatar renderer",
        add_help=False,
        formatter_class=RawTextHelpFormatter,
    )
    parser.add_argument(
        "--help", action="help", help=textwrap.dedent("show this help message and exit")
    )

    default_avatar_path = os.path.realpath(
        os.path.join(__file__, "../../../../../rpm/public/avatar_1_f.glb")
    )
    default_blendshapes_dir = os.path.realpath(
        os.path.join(__file__, "../../../../logs/<DATE TIME>/detectionLog.json")
    )
    ...
    parser.add_argument(
        ...
        type=str,
        help=textwrap.dedent(
            "path to the recorded blendshapes:\n"
    ...
    parser.add_argument(
        ...
        type=str,
        help=textwrap.dedent(
            "path to for the output directory for storing rendered images (required for mode `-m test_store` and `-m test_process`)\n"
            "DEFAULT:\n  " + default_out_dir
        ),
    ...
    def validate_b():
        if (
            not the_blendshapes_data_path
            or not os.path.exists(the_blendshapes_data_path)
            or not os.path.isfile(the_blendshapes_data_path)
        ):
            if not the_blendshapes_data_path:
                reason = "not specified"
            elif not os.path.exists(the_blendshapes_data_path):
                reason = "file does not exist at " + os.path.realpath(
                    the_blendshapes_data_path
                )
            else:
                reason = "path is not file, at " + os.path.realpath(
                    the_blendshapes_data_path
                )
            raise ValueError(
                "invalid path for recorded blendshapes (option `-b`): " + reason
            )
    ...
    match the_mode:
        case "win":
            run_renderer_window(
                the_model_path, the_blendshapes_data_path, headless=is_headless
            )
        case "test_store":
            validate_b()
            test_store_images_for_recorded_blendshapes(
                the_model_path,
                the_blendshapes_data_path,
                out_dir=the_out_dir,
        ...
        case "test_queue":
            validate_b()
            test_render_recorded_blendshapes_to_queue(
                the_model_path, the_blendshapes_data_path, headless=is_headless
            )
        case "test_process":
            validate_b()
            test_render_recorded_blendshapes_in_process(
                the_model_path,
                the_blendshapes_data_path,
                out_dir=the_out_dir,
                headless=is_headless,
        ...
        case _:
            raise ValueError(
                f'unknown mode "{the_mode}": use --help to list available modes'
            )
