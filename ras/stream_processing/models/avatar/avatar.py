...
class Avatar(Converter):
    ...
    def initializeFaceLandmarkerModel(self):
        ...
        face_landmarker_path = os.path.join(
            current_file_path, "face_landmarker.task"
        )
    ...
    def detect_face(self, data: np.ndarray) -> dict:
        """
    ...
    def convert_frame(self, data: np.ndarray) -> np.ndarray:
        ...
        """
        face_detection_results = self.detect_face(data)
        np_out_img = np.zeros(
        ...
        if face_detection_results:
            ...
            if message.startswith("/"):
                ...
                if avatar_height > np_out_img.shape[0] or avatar_width > np_out_img.shape[1]:
                    factor = min(np_out_img.shape[0] / avatar_height, np_out_img.shape[1] / avatar_width)
                    avatar_img = cv2.resize(
                        avatar_img, (int(avatar_width * factor), int(avatar_height * factor))
                    )
                    avatar_height, avatar_width, _ = avatar_img.shape
